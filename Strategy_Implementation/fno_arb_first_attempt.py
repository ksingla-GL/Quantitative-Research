...
access_token='35W1NtosEdp0NpVFsF1V8GCempcF9x3M'
...
def on_ticks(ws, ticks):  # noqa
    ...
    for tick in ticks:
        ...
        token,'Fut Symbol'].values[0] in all_ticks:
            ...
            fut_price=all_ticks[token2]
                        
            #all_ts.loc[len(all_ts)]=[times,token,token2,ltps,fut_price]

            if fut_price-ltps>0.004*ltps:
                ...
                #kws.stop()
                pdb.set_trace()
                #tf.loc[len(tf)]=[times,token,token2,ltps,fut_price]
                if token not in cur_pos and token2 not in cur_pos and \
                kite.margins()['equity']['net']>lot_sizes[token2]*1.3*fut_price:
        ...
        token,'Cash Symbol'].values[0] in all_ticks:
            ...
            cash_sym=cash_map.loc[cash_map[cash_map.columns[1]]==token2,cash_map.columns[0]].values[0]
            fut_sym=fut_map.loc[fut_map[fut_map.columns[1]]==token,fut_map.columns[0]].values[0]
            print(ltpb,ltps)
...
def on_connect(ws, response):  # noqa
    ws.subscribe([7670273,11662338])
    ws.set_mode(ws.MODE_FULL, [7670273,11662338])
...
order_id = kite.place_order(
            ...
            validity=kite.VALIDITY_IOC,
            price=p)
while (len(kite.order_trades(order_id))==0):
    order_id = kite.place_order(
            variety=kite.VARIETY_REGULAR,
            exchange=kite.EXCHANGE_NSE,
            tradingsymbol="NCC",
